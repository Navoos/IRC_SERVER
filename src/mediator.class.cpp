...
// void Mediator::add_channel() {
//     Channel *channel = new Channel("ayoub");
//     this->__channels.insert(channel, )
// }

std::vector<Channel*> Mediator::get_channels(){
    return (__channels);
}

std::vector<std::string> Mediator::split(const std::string& input, const std::string& delimiter) {
...
void Mediator::part_cmd(Client *client, std::vector<Channel*> channel, std::vector<std::string> __cmd) {
    (void)channel;
    std::vector<std::string> cmd_helper;
    if (__cmd.size() == 2 || __cmd.size() == 3) {
        cmd_helper = split(__cmd[1], ",");
        for (size_t i = 0; i < cmd_helper.size(); i++) {
            if (cmd_helper[i].empty())
                break;
            if (cmd_helper[i][0] != '#') {
                client->put_message("10000", ":Bad channel mask");
                return ;
            }
        }
    }
